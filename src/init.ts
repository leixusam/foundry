...
// Get the .ralph directory path, ensuring it exists
function getRalphDir(): string {
...
function saveMcpConfig(apiKey: string): void {
  const mcpPath = join(getRalphDir(), 'mcp.json');
  ...
  console.log('Linear MCP server configured in .ralph/mcp.json');
}

// Save credentials to .ralph/env file and configure MCP
function saveCredentials(apiKey: string, teamKey: string): void {
  const envPath = join(getRalphDir(), 'env');
  const content = `# Ralph configuration (auto-generated by wizard)
LINEAR_API_KEY=${apiKey}
LINEAR_TEAM_KEY=${teamKey}
`;
  writeFileSync(envPath, content, 'utf-8');
  console.log(`\nCredentials saved to .ralph/env`);
...
export async function runInitialization(): Promise<InitResult | undefined> {
  ...
  // Save credentials to .ralph/env if we collected new ones
  if (needsSave) {
