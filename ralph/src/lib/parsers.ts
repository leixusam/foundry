...
export function parseDispatchResult(output: string): DispatchResult | null {
  ...
  const result: Partial<DispatchResult> = {
    existingArtifacts: {},
    labels: [],
  };
  ...
  if (issueIdMatch) result.issueId = issueIdMatch[1].trim();

  const identifierMatch = yamlContent.match(/issue_identifier:\s*(.+)/);
  if (identifierMatch) result.issueIdentifier = identifierMatch[1].trim();

  const titleMatch = yamlContent.match(/issue_title:\s*(.+)/);
  if (titleMatch) result.issueTitle = titleMatch[1].trim();
  ...
  if (timestampMatch) result.claimTimestamp = timestampMatch[1].trim();
  ...
  if (priorityMatch) {
    ...
    if (['urgent', 'high', 'medium', 'low', 'none'].includes(priority)) {
      result.priority = priority as DispatchResult['priority'];
    }
  }
  ...
  if (descMatch) {
    ...
    if (singleLineDesc) result.issueDescription = singleLineDesc[1].trim();
  }
...
export function parseWorkResult(output: string): WorkResult | null {
  ...
  const yamlContent = match[1];
  const result: Partial<WorkResult> = {};
  ...
  if (summaryMatch) {
    ...
    if (singleLineSummary) result.summary = singleLineSummary[1].trim();
  }
...
export function parseLinearUpdate(output: string): LinearUpdateResult | null {
  ...
  const yamlContent = match[1];
  const result: Partial<LinearUpdateResult> = {};
  ...
  return result as LinearUpdateResult;
}
