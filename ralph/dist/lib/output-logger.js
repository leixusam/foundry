...
function getAgentLogPath(agentNumber) {
    ...
    return join(outputDir, loopNameDisplay, `loop-${currentLoopNumber}`, `agent-${agentNumber}.log`);
}
/**
...
function getAgentTerminalLogPath(agentNumber) {
    ...
    const outputDir = getOutputDir();
    const loopNameDisplay = getLoopInstanceNameDisplay(currentLoopName);
    return join(outputDir, loopNameDisplay, `loop-${currentLoopNumber}`, `agent-${agentNumber}-terminal.log`);
}
/**
 ...
 */
export function initLoopLogger(loopInstanceName, loopNumber) {
